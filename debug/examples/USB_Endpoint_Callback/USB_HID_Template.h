...
ESP_EVENT_DECLARE_BASE(ARDUINO_USB_HID_TEMPLATE_EVENTS);

typedef enum
{
    ARDUINO_USB_HID_TEMPLATE_ANY_EVENT = ESP_EVENT_ANY_ID,
    ARDUINO_USB_HID_TEMPLATE_SET_FEATURE_EVENT = 0,
    ARDUINO_USB_HID_TEMPLATE_GET_FEATURE_EVENT,
    ARDUINO_USB_HID_TEMPLATE_OUTPUT_EVENT,
    ARDUINO_USB_HID_TEMPLATE_MAX_EVENT,
} arduino_usb_hid_template_event_t;

typedef struct
{
    const uint8_t *buffer;
    uint16_t len;
} arduino_usb_hid_template_event_data_t;

class USB_HID_TEMPLATE : public USBHIDDevice, public Stream
{
private:
    USBHID hid;
...
public:
    ...
    USB_HID_TEMPLATE(uint8_t report_size = 63, bool prepend_size = false);
    void begin(void);
    void end(void);
    ...
    void onEvent(arduino_usb_hid_template_event_t event, esp_event_handler_t callback);
